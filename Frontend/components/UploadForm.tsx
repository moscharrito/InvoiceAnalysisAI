...
export default function UploadForm({ onUpload }: UploadFormProps) {
  ...
  return (
    <div className="w-full">
      ...
      {!file ? (
        <div
          ...
          className={`
            ...
            ${isDragging ? 'dragover scale-[1.02]' : 'hover:border-amber-400'}
          `}
        >
          ...
          <div className="flex flex-col items-center">
            ...
            `}>
              <svg
                className={`w-8 h-8 transition-all duration-300 ${isDragging ? 'text-amber-700 -translate-y-1' : 'text-amber-600'}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div className="text-center">
              <p className="text-charcoal-800 font-medium mb-1">
                {isDragging ? 'Drop your file here' : 'Drag & drop your invoice'}
              </p>
              <p className="text-charcoal-400 text-sm">
                or <span className="text-amber-600 hover:text-amber-700 transition-colors font-medium">browse files</span>
              </p>
            </div>
        ...
        <div className="glass-card p-6 animate-scale-in">
          <div className="flex items-start gap-4">
            {/* File Icon */}
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-amber-100 to-amber-200 flex items-center justify-center text-amber-600 flex-shrink-0">
              {getFileIcon(file.type)}
            </div>

            {/* File Info */}
            <div className="flex-1 min-w-0">
              <p className="text-charcoal-800 font-medium truncate mb-1">
                {file.name}
              </p>
              <p className="text-charcoal-400 text-sm">
            ...
            {!isProcessing && (
              <button
                onClick={removeFile}
                className="w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 flex items-center justify-center text-red-500 hover:text-red-600 transition-all"
              >
          ...
          {!isProcessing && (
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={triggerFileInput}
                className="flex-1 py-3 px-4 rounded-xl border border-charcoal-200 text-charcoal-600 hover:text-charcoal-800 hover:border-charcoal-300 hover:bg-charcoal-50 transition-all text-sm font-medium"
              >
                Change File
              </button>
          ...
          {/* Processing State */}
          {isProcessing && (
            <div className="mt-4 flex items-center justify-center gap-3 text-charcoal-500 text-sm">
      ...
      {/* Quick Tips */}
      {!file && (
        <div className="mt-6 flex items-start gap-3 text-charcoal-400 text-xs">
